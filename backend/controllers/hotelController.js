...
exports.fetchHotels = async (req, res) => {
    try {
      const hotels = await Hotel.find().populate('images');
...
exports.addHotel = async (req, res) => {
  ...
  try {
    const {
      ...
      phoneNumber, emailAddress, minPricePerNight, maxPricePerNight,
      roomTypes, amenities, numberOfRoomsAvailable, bookingCalendar
    } = req.body;
    ...
    const hotel = new Hotel({
      name, description, location: { address, city, state, country, zipCode },
...
exports.updateHotel = async (req, res) => {
    try {
      ...
      const hotel = await Hotel.findByIdAndUpdate(hotelId, updates, { new: true }).populate('images');
      if (!hotel) {
